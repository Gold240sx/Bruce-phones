...
exports.cleanupFiles = functions.pubsub.schedule("every 24 hours").onRun(async (context) => {
	...
	try {
		...
		await Promise.all(files.filter((file) => file.metadata.timeCreated < ninetyDaysAgo).map((file) => file.delete()))
